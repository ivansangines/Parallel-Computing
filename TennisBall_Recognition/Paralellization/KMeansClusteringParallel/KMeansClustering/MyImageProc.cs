...
namespace KMeansClustering
{
    class MyImageProc
    {
        ...
        public static bool DrawClusters(PictureBox pan, List<MyPoint> PList, double scale, int clusterCount)
        {
            ...
            Parallel.For(0, PList.Count, (index) =>
            //for (int i = 1; i < PList.Count; i++)
            {
                Graphics dc = pan.CreateGraphics(); //no need of lock because each thread has its own graphics???
                Point p2 = new Point { X = (int)PList[index].X, Y = (int)PList[index].Y };

                Point p1a = new Point((int)(p2.X * scale) - 2, (int)(p2.Y * scale) - 2);
                Point p2a = new Point((int)(p2.X * scale) + 2, (int)(p2.Y * scale) + 2);
                //lock (olock)
                //{                    
                    dc.DrawLine(new Pen(CColors[PList[index].ClusterId], 1), p1a, p2a);
                //}

                Point p3a = new Point((int)(p2.X * scale) + 2, (int)(p2.Y * scale) - 2);
                Point p4a = new Point((int)(p2.X * scale) - 2, (int)(p2.Y * scale) + 2);
                //lock (olock)
                //{                    
                    dc.DrawLine(new Pen(CColors[PList[index].ClusterId]), p3a, p4a);
                //}
            });
            return true;
        }
